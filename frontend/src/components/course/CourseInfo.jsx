...
const CourseInfo = ({ courseData, user, isEnrolled = false, onEnroll, enrolling = false }) => {
  ...
  return (
    <div className="flex  w-full flex-row h-[15vh] py-3 px-5 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl justify-between transition-all duration-100">
      {/* Left: Course info */}
      ...
      <div className="flex gap-3 items-center full-h">
        {user?.role === "Instructor" && (
          <button className="px-4 py-4 bg-blue-600 text-white rounded-lg shadow hover:bg-blue-700 transition">
            📤 Upload bài học
          </button>
        )}
        ...
        {user?.role && user.role.includes("Student") && (
          !isEnrolled ? (
            ...
            <button className="px-4 py-4 bg-green-600 text-white rounded-lg shadow hover:bg-green-700 transition">
              📝 Làm Quiz
            </button>
          )
        )}
